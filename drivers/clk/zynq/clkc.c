...
#define SLCR_ARMPLL_CTRL		0x100
#define SLCR_DDRPLL_CTRL		0x104
#define SLCR_IOPLL_CTRL			0x108
#define SLCR_PLL_STATUS			0x10c
#define SLCR_ARM_CLK_CTRL		0x120
#define SLCR_DDR_CLK_CTRL		0x124
#define SLCR_DCI_CLK_CTRL		0x128
#define SLCR_APER_CLK_CTRL		0x12c
#define SLCR_GEM0_CLK_CTRL		0x140
#define SLCR_GEM1_CLK_CTRL		0x144
#define SLCR_SMC_CLK_CTRL		0x148
#define SLCR_LQSPI_CLK_CTRL		0x14c
#define SLCR_SDIO_CLK_CTRL		0x150
#define SLCR_UART_CLK_CTRL		0x154
#define SLCR_SPI_CLK_CTRL		0x158
#define SLCR_CAN_CLK_CTRL		0x15c
#define SLCR_CAN_MIOCLK_CTRL		0x160
#define SLCR_DBG_CLK_CTRL		0x164
#define SLCR_PCAP_CLK_CTRL		0x168
#define SLCR_FPGA0_CLK_CTRL		0x170
#define SLCR_621_TRUE			0x1c4
#define SLCR_SWDT_CLK_SEL		0x304
...
static void __init zynq_clk_setup(struct device_node *np)
{
	...
	struct device_node *sclrnp =
		of_find_compatible_node(NULL, NULL, "xlnx,zynq-slcr");
	void __iomem *slcr_base = of_iomap(sclrnp, 0);
	...
	/* PLLs */
	clk = clk_register_zynq_pll("armpll_int", "ps_clk",
			slcr_base + SLCR_ARMPLL_CTRL,
			slcr_base + SLCR_PLL_STATUS, 0, &armpll_lock);
	clks[armpll] = clk_register_mux(NULL, clk_output_name[armpll],
			armpll_parents, 2, CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_ARMPLL_CTRL, 4, 1, 0, &armpll_lock);

	clk = clk_register_zynq_pll("ddrpll_int", "ps_clk",
			slcr_base + SLCR_DDRPLL_CTRL,
			slcr_base + SLCR_PLL_STATUS, 1, &ddrpll_lock);
	clks[ddrpll] = clk_register_mux(NULL, clk_output_name[ddrpll],
			ddrpll_parents, 2, CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_DDRPLL_CTRL, 4, 1, 0, &ddrpll_lock);

	clk = clk_register_zynq_pll("iopll_int", "ps_clk",
			slcr_base + SLCR_IOPLL_CTRL,
			slcr_base + SLCR_PLL_STATUS, 2, &iopll_lock);
	clks[iopll] = clk_register_mux(NULL, clk_output_name[iopll],
			iopll_parents, 2, CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_IOPLL_CTRL, 4, 1, 0, &iopll_lock);

	/* CPU clocks */
	tmp = readl(slcr_base + SLCR_621_TRUE) & 1;
	clk = clk_register_mux(NULL, "cpu_mux", cpu_parents, 4,
			CLK_SET_RATE_NO_REPARENT, slcr_base + SLCR_ARM_CLK_CTRL,
			4, 2, 0, &armclk_lock);
	clk = clk_register_divider(NULL, "cpu_div", "cpu_mux", 0,
			slcr_base + SLCR_ARM_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&armclk_lock);

	clks[cpu_6or4x] = clk_register_gate(NULL, clk_output_name[cpu_6or4x],
			"cpu_div", CLK_SET_RATE_PARENT | CLK_IGNORE_UNUSED,
			slcr_base + SLCR_ARM_CLK_CTRL, 24, 0, &armclk_lock);
	...
	clks[cpu_3or2x] = clk_register_gate(NULL, clk_output_name[cpu_3or2x],
			...
			slcr_base + SLCR_ARM_CLK_CTRL, 25, 0, &armclk_lock);

	clk = clk_register_fixed_factor(NULL, "cpu_2x_div", "cpu_div", 0, 1,
			2 + tmp);
	clks[cpu_2x] = clk_register_gate(NULL, clk_output_name[cpu_2x],
			"cpu_2x_div", CLK_IGNORE_UNUSED,
			slcr_base + SLCR_ARM_CLK_CTRL, 26, 0, &armclk_lock);

	clk = clk_register_fixed_factor(NULL, "cpu_1x_div", "cpu_div", 0, 1,
			4 + 2 * tmp);
	clks[cpu_1x] = clk_register_gate(NULL, clk_output_name[cpu_1x],
			"cpu_1x_div", CLK_IGNORE_UNUSED,
			slcr_base + SLCR_ARM_CLK_CTRL, 27, 0, &armclk_lock);
	...
	clks[swdt] = clk_register_mux(NULL, clk_output_name[swdt],
			...
			slcr_base + SLCR_SWDT_CLK_SEL, 0, 1, 0,	&swdtclk_lock);

	/* DDR clocks */
	clk = clk_register_divider(NULL, "ddr2x_div", "ddrpll", 0,
			slcr_base + SLCR_DDR_CLK_CTRL, 26, 6,
			CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&ddrclk_lock);
	clks[ddr2x] = clk_register_gate(NULL, clk_output_name[ddr2x],
			"ddr2x_div", 0,
			slcr_base + SLCR_DDR_CLK_CTRL, 1, 0, &ddrclk_lock);
	clk_prepare_enable(clks[ddr2x]);
	clk = clk_register_divider(NULL, "ddr3x_div", "ddrpll", 0,
			slcr_base + SLCR_DDR_CLK_CTRL, 20, 6,
			CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&ddrclk_lock);
	clks[ddr3x] = clk_register_gate(NULL, clk_output_name[ddr3x],
			"ddr3x_div", 0,
			slcr_base + SLCR_DDR_CLK_CTRL, 0, 0, &ddrclk_lock);
	clk_prepare_enable(clks[ddr3x]);

	clk = clk_register_divider(NULL, "dci_div0", "ddrpll", 0,
			slcr_base + SLCR_DCI_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED |	CLK_DIVIDER_ALLOW_ZERO,
			&dciclk_lock);
	clk = clk_register_divider(NULL, "dci_div1", "dci_div0",
			CLK_SET_RATE_PARENT, slcr_base + SLCR_DCI_CLK_CTRL,
			20, 6, CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&dciclk_lock);
	...
	for (i = fclk0; i <= fclk3; i++)
		zynq_clk_register_fclk(i, clk_output_name[i],
			slcr_base + SLCR_FPGA0_CLK_CTRL + 0x10 * (i - fclk0),
			periph_parents);

	zynq_clk_register_periph_clk(lqspi, 0, clk_output_name[lqspi], NULL,
			slcr_base + SLCR_LQSPI_CLK_CTRL, periph_parents, 0);

	zynq_clk_register_periph_clk(smc, 0, clk_output_name[smc], NULL,
			slcr_base + SLCR_SMC_CLK_CTRL, periph_parents, 0);

	zynq_clk_register_periph_clk(pcap, 0, clk_output_name[pcap], NULL,
			slcr_base + SLCR_PCAP_CLK_CTRL, periph_parents, 0);

	zynq_clk_register_periph_clk(sdio0, sdio1, clk_output_name[sdio0],
			clk_output_name[sdio1], slcr_base + SLCR_SDIO_CLK_CTRL,
			periph_parents, 1);

	zynq_clk_register_periph_clk(uart0, uart1, clk_output_name[uart0],
			clk_output_name[uart1], slcr_base + SLCR_UART_CLK_CTRL,
			periph_parents, 1);

	zynq_clk_register_periph_clk(spi0, spi1, clk_output_name[spi0],
			clk_output_name[spi1], slcr_base + SLCR_SPI_CLK_CTRL,
			periph_parents, 1);
	...
	clk = clk_register_mux(NULL, "gem0_mux", periph_parents, 4,
			CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_GEM0_CLK_CTRL, 4, 2, 0, &gem0clk_lock);
	clk = clk_register_divider(NULL, "gem0_div0", "gem0_mux", 0,
			slcr_base + SLCR_GEM0_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED |	CLK_DIVIDER_ALLOW_ZERO,
			&gem0clk_lock);
	clk = clk_register_divider(NULL, "gem0_div1", "gem0_div0",
			CLK_SET_RATE_PARENT, slcr_base + SLCR_GEM0_CLK_CTRL,
			20, 6, CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&gem0clk_lock);
	clk = clk_register_mux(NULL, "gem0_emio_mux", gem0_mux_parents, 2,
			CLK_SET_RATE_PARENT | CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_GEM0_CLK_CTRL, 6, 1, 0, &gem0clk_lock);
	clks[gem0] = clk_register_gate(NULL, clk_output_name[gem0],
			"gem0_emio_mux", CLK_SET_RATE_PARENT,
			slcr_base + SLCR_GEM0_CLK_CTRL, 0, 0, &gem0clk_lock);
	...
	clk = clk_register_mux(NULL, "gem1_mux", periph_parents, 4,
			CLK_SET_RATE_NO_REPARENT, slcr_base + SLCR_GEM1_CLK_CTRL,
			4, 2, 0, &gem1clk_lock);
	clk = clk_register_divider(NULL, "gem1_div0", "gem1_mux", 0,
			slcr_base + SLCR_GEM1_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED |	CLK_DIVIDER_ALLOW_ZERO,
			&gem1clk_lock);
	clk = clk_register_divider(NULL, "gem1_div1", "gem1_div0",
			CLK_SET_RATE_PARENT, slcr_base + SLCR_GEM1_CLK_CTRL,
			20, 6, CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&gem1clk_lock);
	clk = clk_register_mux(NULL, "gem1_emio_mux", gem1_mux_parents, 2,
			CLK_SET_RATE_PARENT | CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_GEM1_CLK_CTRL, 6, 1, 0, &gem1clk_lock);
	clks[gem1] = clk_register_gate(NULL, clk_output_name[gem1],
			"gem1_emio_mux", CLK_SET_RATE_PARENT,
			slcr_base + SLCR_GEM1_CLK_CTRL, 0, 0, &gem1clk_lock);
	...
	clk = clk_register_mux(NULL, "can_mux", periph_parents, 4,
			CLK_SET_RATE_NO_REPARENT, slcr_base + SLCR_CAN_CLK_CTRL,
			4, 2, 0, &canclk_lock);
	clk = clk_register_divider(NULL, "can_div0", "can_mux", 0,
			slcr_base + SLCR_CAN_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED |	CLK_DIVIDER_ALLOW_ZERO,
			&canclk_lock);
	clk = clk_register_divider(NULL, "can_div1", "can_div0",
			CLK_SET_RATE_PARENT, slcr_base + SLCR_CAN_CLK_CTRL,
			20, 6, CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
			&canclk_lock);
	...
	clk = clk_register_mux(NULL, "can0_mio_mux",
			...
			CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_CAN_MIOCLK_CTRL, 0, 6, 0,
			&canmioclk_lock);
	clk = clk_register_mux(NULL, "can1_mio_mux",
			can_mio_mux_parents, 54, CLK_SET_RATE_PARENT |
			CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_CAN_MIOCLK_CTRL, 16, 6,
			0, &canmioclk_lock);
	clks[can0] = clk_register_mux(NULL, clk_output_name[can0],
			can0_mio_mux2_parents, 2, CLK_SET_RATE_PARENT |
			CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_CAN_MIOCLK_CTRL, 6, 1, 0,
			&canmioclk_lock);
	clks[can1] = clk_register_mux(NULL, clk_output_name[can1],
			can1_mio_mux2_parents, 2, CLK_SET_RATE_PARENT |
			CLK_SET_RATE_NO_REPARENT,
			slcr_base + SLCR_CAN_MIOCLK_CTRL, 22, 1,
			0, &canmioclk_lock);
	...
	clk = clk_register_mux(NULL, "dbg_mux", periph_parents, 4,
			CLK_SET_RATE_NO_REPARENT, slcr_base + SLCR_DBG_CLK_CTRL,
			4, 2, 0, &dbgclk_lock);
	clk = clk_register_divider(NULL, "dbg_div", "dbg_mux", 0,
			slcr_base + SLCR_DBG_CLK_CTRL, 8, 6,
			CLK_DIVIDER_ONE_BASED |	CLK_DIVIDER_ALLOW_ZERO,
			&dbgclk_lock);
